function [xSc, ySc, zSc,IRRs,graphColor] = diffcolbylam(wL)
%DIFFCOLBYLAM 此处显示有关此函数的摘要
%   此处显示详细说明
...
% INPUTS --------------------------------------------------------
num = 2501;                    % No. observation points [501]
...
axis([xs1 xs2 0 1.1]);
xlabel('位置  (mm)','fontSize',fs);
ylabel('辐照度  ( W.m^{-2} )','fontSize',fs);

mainTitle  = sprintf('双缝');
pT1 = '\\lambda = ';
pT2 = num2str(wL*1e9,'%3.1f');
pT3 = ' nm    b = ';
pT4 = num2str(b,'%3.1e');
pT5 = ' m    a = ';
pT6 = num2str(a,'%3.1e');
